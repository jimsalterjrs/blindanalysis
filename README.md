...
There are no prerequisites or dependencies beyond Perl itself: just download it, make sure it's executable, and run it.  On Linux or BSD or Mac, you should already have Perl, and I'd recommend putting it in `/usr/local/bin` and `chmod 755 /usr/local/bin/blindrename.pl`, after which you can just type **blindrename.pl** ***foldername*** and it will obligingly do its thing.  On Windows... well, do whatever you need to do to run a Perl script on Windows.  (If you're a Windows-only person and feeling out of your depth: you can check out <a href="http://www.activestate.com/activeperl" target="_blank">ActivePerl</a> for a Windows implementations of Perl, but it may be easier to just install and use Microsoft's Windows Subsystem for Linux.)
